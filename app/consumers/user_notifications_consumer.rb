class UserNotificationsConsumer < ApplicationConsumer
  # user_notifications トピックからユーザー通知を処理
  ...
  def consume
    messages.each do |message|
      begin
        ...
        puts "=" * 60
        puts "📨 USER NOTIFICATION RECEIVED"
        puts "   ユーザーID: #{user_id}"
        puts "   通知タイプ: #{payload['type']}"
        ...
        puts "❌ [USER_NOTIFICATIONS] JSON Parse Error: #{e.message}"
        puts "   Raw payload: #{message.payload.inspect}"
      rescue => e
        puts "❌ [USER_NOTIFICATIONS] Processing Error: #{e.message}"
